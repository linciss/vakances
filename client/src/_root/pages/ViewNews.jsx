...
const ViewNews = () => {
  ...
  const relatedNewsItems = newsItems.filter(item => item.id !== parseInt(id)).slice(0, 2);
    ...
    <div className="container max-w-[1280px] mx-auto py-12">
      ...
      <div className="flex flex-col lg:flex-row mt-6">
        <div className="lg:w-2/3">
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">{newsItem.title}</h1>
          <p className="mt-4 text-lg leading-8 text-gray-600">{newsItem.description}</p>
        </div>
        <div className="lg:w-1/3 lg:pl-8 mt-12 lg:mt-0">
          <h2 className="text-2xl font-bold tracking-tight text-gray-900">Citi jaunumi</h2>
          <div className="grid gap-8 mt-6">
            {relatedNewsItems.map(relatedNewsItem => (
              ...
              >
                <div className="w-full h-48 bg-center bg-cover rounded-t-box" style={{ backgroundImage: `url(${relatedNewsItem.imageUrl})` }}></div>
                <div className="flex items-center gap-x-4 text-xs mt-2">
                  <time dateTime={relatedNewsItem.timeCreated} className="text-gray-500">
                ...
                <div className="group relative">
                  <h3 className="mt-2 text-lg font-semibold leading-6 text-gray-900 group-hover:text-gray-600">
                    <Link to={`/viewNews/${relatedNewsItem.id}`}>
